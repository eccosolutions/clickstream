...
public class ActiveStreamsServlet extends HttpServlet {
	...
	private void printClickstreamDetail(HttpServletRequest request, PrintWriter out, String sid) {
		...
		Map<String, Clickstream> clickstreams = (Map<String, Clickstream>)getServletContext().getAttribute(
				ClickstreamListener.CLICKSTREAMS_ATTRIBUTE_KEY);
		...
		synchronized (stream) {
			...
			for (ClickstreamRequest csr : stream.getStream()) {
				String click = csr.toString();
		...
		if (clickstreams.isEmpty()) {
			...
			synchronized (clickstreams) {
				...
				for (Map.Entry<String, Clickstream> entry : clickstreams.entrySet()) {
					...
					String hostname = (stream.getHostname() != null && !"".equals(stream.getHostname()) ?
